...
export const useSonificationEngine = (
  ...
  const transportEnabledRef = useRef(false);
  const activePluginsRef = useRef<EngineStartData | null>(null);
  ...
  const flushTransport = useCallback(() => {
    const flushResult = sonifierHandleRef.current?.sonifier.processSamples(new Map());
  ...
  const disposeActiveHandles = useCallback(() => {
    ...
    activePluginsRef.current = null;
    transportEnabledRef.current = false;
  ...
  const start = useCallback(
    async (options?: StartOptions): Promise<SonificationEngineStartResult> => {
      const enableTransport = options?.transport !== "off";
      const desiredStatus = enableTransport ? "running" : "idle";
      ...
      const resolvedPluginIds: EngineStartData = {
      ...
      const currentPluginIds = activePluginsRef.current;
      const hasActiveHandles =
        detectorHandleRef.current !== null &&
        samplerHandleRef.current !== null &&
        sonifierHandleRef.current !== null;
      const samePlugins =
        currentPluginIds?.detectionPluginId === resolvedPluginIds.detectionPluginId &&
        currentPluginIds?.samplingPluginId === resolvedPluginIds.samplingPluginId &&
        currentPluginIds?.sonificationPluginId === resolvedPluginIds.sonificationPluginId;

      // Fast-path: same plugin set is already initialized, only toggle transport.
      if (hasActiveHandles && samePlugins) {
        transportEnabledRef.current = enableTransport;
        if (!enableTransport) {
          flushTransport();
        }
        setStatusIfCurrent({ status: desiredStatus });
        return {
          status: desiredStatus,
          data: resolvedPluginIds,
        };
      }

      // Ensure at most one active engine session (covers plugin switches + restart).
      ...
      dh.detector.onPointsDetected((points) => {
        if (!transportEnabledRef.current) {
          return;
        }
      ...
      activePluginsRef.current = resolvedPluginIds;
      transportEnabledRef.current = enableTransport;
      if (!enableTransport) {
        flushTransport();
      }
    ...
    [config, disposeActiveHandles, flushTransport, imageCanvasRef, imageOverlayRef, setStatus],
  );

  const stop = useCallback(() => {
    // Invalidate any in-flight start() chain and keep handles alive.
    startSessionIdRef.current += 1;
    transportEnabledRef.current = false;
    flushTransport();
    setStatus({ status: "idle" });
  }, [flushTransport, setStatus]);
